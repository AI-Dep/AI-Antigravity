...
def main():
    ...
    if args.no_rpa:
        ...
        try:
            ...
            # Confirm manual login is complete
            print()
            print("=" * 70)
            print("⚠️  CRITICAL: Manual Login Verification")
            print("=" * 70)
            print()
            print("RPA CANNOT automate the FA CS login process.")
            print("You MUST have manually completed ALL of these steps:")
            print()
            print("  ✓ Windows Security credential entry")
            print("  ✓ RemoteApp session configuration")
            print("  ✓ FA CS Sign-In button click")
            print("  ✓ Thomson Reuters browser login")
            print("  ✓ Email/Password entry")
            print("  ✓ MFA verification code (from your phone)")
            print()
            print("Verify FA CS is FULLY LOGGED IN:")
            print("  ✓ FA CS main window is visible")
            print("  ✓ No login prompts are showing")
            print("  ✓ You can see client data / main menu")
            print()
            print("See FA_CS_LOGIN_LIMITATIONS.md for details on why login cannot be automated.")
            print("=" * 70)
            print()

            response = input("Have you completed the ENTIRE manual login process? (yes/no): ").strip().lower()
            if response not in ['yes', 'y']:
                print()
                print("❌ RPA automation cancelled.")
                print("   Please complete the manual login process first.")
                print("   See FA_CS_LOGIN_LIMITATIONS.md for step-by-step instructions.")
                sys.exit(0)
            ...
            input("Press Enter to start automation (Ctrl+C to cancel)...")
            print()
