...
def calculate_section_1245_recapture(
    ...
    accum_includes_179_bonus: bool = True
) -> Dict[str, float]:
    """
    ...
    Args:
        ...
        accumulated_depreciation: Depreciation taken (may or may not include 179/bonus)
        proceeds: Sale proceeds
        section_179_taken: Section 179 expensing taken (only if NOT in accumulated_depreciation)
        bonus_taken: Bonus depreciation taken (only if NOT in accumulated_depreciation)
        accum_includes_179_bonus: If True (default), accumulated_depreciation already includes
            Section 179 and bonus amounts, so they won't be added again. Set to False only
            if you're passing MACRS-only accumulated depreciation separately from 179/bonus.
    ...
    CRITICAL: Most fixed asset systems (including FA CS) track accumulated depreciation
    as a single total that INCLUDES Section 179 and bonus. In that case, leave
    accum_includes_179_bonus=True and don't pass section_179_taken/bonus_taken separately
    to avoid double-counting.
    """
    # CRITICAL FIX: Prevent double-counting of Section 179 and Bonus
    # Most systems include 179/bonus in accumulated_depreciation already
    if accum_includes_179_bonus:
        # accumulated_depreciation already includes everything
        total_depreciation = accumulated_depreciation
        # Warn if caller also passed 179/bonus separately (likely a mistake)
        if section_179_taken > 0 or bonus_taken > 0:
            import warnings
            warnings.warn(
                "Section 179/Bonus amounts passed separately but accum_includes_179_bonus=True. "
                "These values will be ignored to prevent double-counting. "
                "Set accum_includes_179_bonus=False if accumulated_depreciation is MACRS-only.",
                UserWarning
            )
    else:
        # Caller explicitly says accumulated_depreciation is MACRS-only
