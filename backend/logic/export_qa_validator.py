...
def validate_fixed_asset_cs_export(
    ...
    if verbose:
        ...
        if len(all_errors) == 0:
            print("[OK] VALIDATION PASSED - Export file is PERFECT QUALITY")
            print("   Ready for Fixed Asset CS import and RPA automation")
        else:
            print(f"[WARNING] VALIDATION ISSUES FOUND: {len(all_errors)} total")
            print()

            if critical:
                print(f"[CRITICAL] CRITICAL ({len(critical)}) - MUST FIX BEFORE RPA:")
            ...
            if errors:
                print(f"[ERROR] ERRORS ({len(errors)}) - SHOULD FIX:")
                for e in errors[:10]:
                    print(f"   {e}")
                if len(errors) > 10:
            ...
            if warnings:
                print(f"[WARNING] WARNINGS ({len(warnings)}) - REVIEW:")
                for e in warnings[:5]:
                    print(f"   {e}")
                if len(warnings) > 5:
            ...
            if is_valid:
                print("[OK] NO CRITICAL/ERROR ISSUES - Safe for RPA (review warnings)")
            else:
                print("[FAILED] CRITICAL/ERROR ISSUES DETECTED - NOT SAFE FOR RPA")
