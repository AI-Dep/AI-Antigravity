...
# ==============================================================================
# MULTI-ENTITY BATCH IMPORT API
# ==============================================================================

@app.post("/upload/multi-entity")
async def upload_multi_entity(
    ...
    """
    Upload a consolidated Trial Balance with multiple entities.

    The TB file should have an "Entity" or "Company" column that identifies
    each CFC entity. The system will:
    1. Parse the entire TB file
    2. Group accounts by entity
    3. Create separate sessions for each entity
    4. Classify accounts for each entity

    Returns:
        List of created sessions with entity info and account counts
    """
    # Validate file
    ...
    if not file.filename.lower().endswith(('.xlsx', '.xls', '.csv')):
        raise api_error(400, "INVALID_FILE_TYPE", "Only Excel/CSV files are supported")
    ...
    if file_size > MAX_UPLOAD_SIZE_BYTES:
        raise api_error(
            413, "FILE_TOO_LARGE",
            f"File size ({file_size / 1024 / 1024:.1f}MB) exceeds maximum ({MAX_UPLOAD_SIZE_MB}MB)"
        )

    # Save to temp file
    temp_fd, temp_file = tempfile.mkstemp(suffix='.xlsx', prefix='facs_multi_')

    try:
        with os.fdopen(temp_fd, 'wb') as buffer:
            buffer.write(file_content)

        # Parse the file to get all accounts
        all_assets = importer.parse_excel(temp_file)
        parse_report = importer.get_last_parse_report()

        if not all_assets:
            raise api_error(400, "NO_DATA", "No accounts found in the uploaded file")

        # Group assets by entity
        entities: Dict[str, List[Asset]] = {}
        no_entity_assets: List[Asset] = []

        for asset in all_assets:
            entity_name = asset.entity or "Unknown Entity"
            if entity_name == "Unknown Entity" or not asset.entity:
                no_entity_assets.append(asset)
            else:
                if entity_name not in entities:
                    entities[entity_name] = []
                entities[entity_name].append(asset)

        # If no entity column found, put all in one session
        if not entities and no_entity_assets:
            entities["Default"] = no_entity_assets
            no_entity_assets = []
        elif no_entity_assets and entities:
            # Add "Unknown Entity" group for accounts without entity
            entities["Unknown Entity"] = no_entity_assets

        if not entities:
            raise api_error(400, "NO_ENTITIES", "No entities found in the file")

        # Create sessions for each entity
        manager = get_session_manager()
        created_sessions = []

        for entity_name, entity_assets in entities.items():
            # Generate a unique session ID for this entity
            timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
            clean_entity = "".join(c for c in entity_name if c.isalnum() or c in ('_', '-'))[:20]
            session_id = f"{timestamp}_{len(entity_assets)}_{clean_entity}"
            ...
            # Classify assets for this entity
            classified_assets = classifier.classify_batch(entity_assets)
            ...
            # Get unique currency from this entity's assets
            currencies = set(a.currency for a in entity_assets if a.currency)
            currency = list(currencies)[0] if len(currencies) == 1 else "USD"

            # Store session metrics
            session.session_metrics = {
                "entity_name": entity_name,
                "currency": currency,
                "total_accounts": len(entity_assets),
                "high_confidence_count": sum(1 for a in classified_assets if a.confidence_score >= 0.7),
                "low_confidence_count": sum(1 for a in classified_assets if a.confidence_score < 0.4),
            }
            ...
            created_sessions.append({
                ...
                "entity_name": entity_name,
                "currency": currency,
                "account_count": len(entity_assets),
                "high_confidence": session.session_metrics["high_confidence_count"],
                "low_confidence": session.session_metrics["low_confidence_count"],
            })

            logger.info(f"[Multi-Entity] Created session {session_id} for entity '{entity_name}' with {len(entity_assets)} accounts")

        logger.info(f"[Multi-Entity] Created {len(created_sessions)} sessions from file '{file.filename}'")

        return {
            "message": f"Successfully created {len(created_sessions)} entity sessions",
            "source_file": file.filename,
            "total_accounts": len(all_assets),
        ...
        logger.error(f"[Multi-Entity] Error processing file: {e}", exc_info=True)
        raise api_error(500, "PROCESSING_ERROR", f"Error processing file: {str(e)}")
...
async def list_sessions(
    ...
    Returns sessions with their entity info and status.
    """
    manager = get_session_manager()

    # Get all sessions (manager will filter by user if auth is enabled)
    all_sessions = []

    # Load sessions from disk
    if manager.session_dir.exists():
        for session_file in manager.session_dir.glob("*.json"):
            try:
                session = manager._load_session(session_file.stem)
                if session:
                    # Check user ownership if auth enabled
                    if user and session.user_id and session.user_id != user.user_id:
                        continue

                    entity_name = session.session_metrics.get("entity_name", "Unknown")
                    currency = session.session_metrics.get("currency", "USD")

                    all_sessions.append({
                        "session_id": session.session_id,
                        "entity_name": entity_name,
                        "currency": currency,
                        "account_count": len(session.assets),
                        "approved_count": len(session.approved_assets),
                        "created_at": session.created_at.isoformat(),
                        "last_accessed": session.last_accessed.isoformat(),
                        "filename": session.last_upload_filename,
                        "status": "TB Loaded" if session.assets else "Empty",
                    })
            except Exception as e:
