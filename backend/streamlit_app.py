...
from backend.logic.tax_year_config import (
...
try:
    from backend.logic.login_ui import (
...
try:
    ...
    try:
        from backend.logic.smart_tab_analyzer import TabRole, TabAnalysis, TabAnalysisResult
    ...
    from backend.logic.client_mapping_manager import (
        ClientMappingManager, generate_mapping_preview, get_manager, reload_manager
    )
    from backend.logic.macrs_classification import (
        load_rules,
        load_overrides,
        save_overrides,
        classify_asset,
    ...
    try:
        from backend.logic.macrs_classification import classify_assets_batch
        BATCH_CLASSIFICATION_AVAILABLE = True
    except ImportError:
        classify_assets_batch = None
        BATCH_CLASSIFICATION_AVAILABLE = False

    from backend.logic.transaction_classifier import (
        classify_all_transactions,
        validate_transaction_classification,
    )
    from backend.logic.fa_export import export_fa_excel, build_fa
...
from backend.logic.validators import validate_assets
from backend.logic.advanced_validations import advanced_validations
from backend.logic.outlier_detector import detect_outliers
from backend.logic.explanations import build_explanation
from backend.logic.sanitizer import sanitize_asset_description
from backend.logic.strategy_config import get_strategy_labels, get_strategy_help

# =====================================================================
# ENHANCED VALIDATION MODULES (Phases 1 & 2)
# =====================================================================
# Phase 1: Existing modules now integrated
from backend.logic.data_quality_score import calculate_data_quality_score, generate_quality_report, DataQualityScore
from backend.logic.rollforward_reconciliation import reconcile_rollforward, generate_rollforward_report
from backend.logic.risk_engine import evaluate_asset_risk
from backend.logic.data_validator import AssetDataValidator, ValidationError

# Phase 2: New tax correctness modules
from backend.logic.classification_verifier import verify_classifications, ClassificationIssue
from backend.logic.prior_year_reconciler import reconcile_to_prior_year, reconcile_cost_totals
from backend.logic.spot_checker import select_spot_check_sample, generate_spot_check_report
from backend.logic.confidence_gate import check_confidence_gate, get_assets_requiring_review

# CPA Planning & Analysis modules
from backend.logic.depreciation_projection import (
    project_portfolio_depreciation, create_detailed_projection_table
)
from backend.logic.recapture import recapture_analysis, calculate_section_1245_recapture, determine_recapture_type
from backend.logic.section_179_carryforward import calculate_section_179_with_income_limit
from backend.logic.materiality import compute_materiality

# Database integration for classification persistence
try:
    from backend.logic.workflow_integration import WorkflowIntegration
    DB_AVAILABLE = True
except Exception:
...
RPA_AVAILABLE = False
try:
    from backend.logic.ai_rpa_orchestrator import AIRPAOrchestrator
    from backend.logic.rpa_fa_cs import test_fa_cs_connection
    RPA_AVAILABLE = True
except Exception:
...
with st.sidebar:
    ...
    if "Manage" in selected_page:
        ...
        try:
            from backend.logic.management_ui import render_management_ui
    ...
    with st.expander("Save/Load Session"):
        try:
            from backend.logic.session_persistence import (
        ...
        with st.spinner("⏳ Preparing export preview..."):
            try:
                ...
                if df is None or df.empty:
                    ...
                    from backend.logic.fa_export import build_fa
...
if "classified_df" in st.session_state:
    ...
    from backend.logic.export_qa_validator import validate_fixed_asset_cs_export
    ...
    if st.button("📊 Generate Depreciation Report", type="primary", disabled=(not ready_to_export or export_disabled)):
        ...
        try:
            if df is None or df.empty:
                ...
                from backend.logic.fa_export import export_fa_excel
